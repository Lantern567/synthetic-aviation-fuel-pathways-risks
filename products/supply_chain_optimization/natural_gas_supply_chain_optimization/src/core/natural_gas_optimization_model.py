...
# 日志文件输出将在优化器初始化时挂载（支持按场景分类）
# 注释掉模块级别的日志挂载，改为在 __init__ 中根据 log_subdir 动态挂载
# try:
...
class NaturalGasSupplyChainOptimizer:
    ...
    def _get_output_path(self, file_type: str, timestamp: str = None) -> str:
        ...
        try:
            # 获取结果基础目录
            results_base_dir = self._get_data_path('output_paths.results_base_dir',
                                            'products/supply_chain_optimization/natural_gas_supply_chain_optimization/results')

            # 添加log_subdir子目录（如果不是default）
            if hasattr(self, 'log_subdir') and self.log_subdir != 'default':
                results_dir = os.path.join(results_base_dir, self.log_subdir)
            else:
            ...
            if hasattr(self, 'log_subdir') and self.log_subdir != 'default':
                results_dir = os.path.join(results_dir, self.log_subdir)
            os.makedirs(results_dir, exist_ok=True)
    ...
    def _get_results_dir(self) -> str:
        ...
        try:
            ...
            if hasattr(self, 'log_subdir') and self.log_subdir != 'default':
                default_dir = os.path.join(default_dir, self.log_subdir)
    ...
    def __init__(self, config_path: str = None, log_subdir: str = None, **override_params):
        """
        初始化优化器

        Args:
            config_path: 配置文件路径，默认使用项目内置配置文件
            log_subdir: 日志和结果子目录（如 'byproduct_hydrogen/one_step'），None时使用默认
            **override_params: 可以通过关键字参数覆盖配置文件中的任何参数
        """
        # 保存日志子目录参数（在加载配置前设置，以便日志系统使用）
        self.log_subdir = log_subdir if log_subdir else "default"

        # 加载配置文件
        self.config = self._load_config(config_path, override_params)

        # 挂载文件日志系统（根据log_subdir动态设置日志路径）
        try:
            _base_dir = get_project_base_dir()
            _log_dir = os.path.join(
                _base_dir,
                ...
                "results",
                "logs",
                self.log_subdir  # 添加子目录
            )
            # 创建日志目录
            os.makedirs(_log_dir, exist_ok=True)

            # 生成日志文件前缀（将路径分隔符替换为下划线）
            filename_prefix = f"ng_supply_chain_{self.log_subdir.replace('/', '_')}"
            mount_file_logging(_log_dir, filename_prefix=filename_prefix)
            logger.info(f"日志文件路径: {_log_dir}/{filename_prefix}_*.log")
        except Exception as e:
            # 如果文件日志挂载失败，输出警告但不中断程序
            print(f"警告：文件日志挂载失败: {e}")
            print("将继续使用控制台日志")
        ...
        try:
            ...
            if is_byproduct_h2:
                ...
                if cache_manager.is_cache_valid('renewable_plants', temp_renewable_file):
                    logger.info("使用缓存的可再生能源数据（100km过滤）")
    ...
    def _filter_renewable_data(self, renewable_data: pd.DataFrame, cache_manager, temp_file: str) -> pd.DataFrame:
        ...
        for plant_name in renewable_data['plant_name'].unique():
            ...
            if len(plant_data) > 0:
                ...
                # 检查坐标是否在北京100公里范围内
                if is_within_beijing_range(plant_lat, plant_lon, 100):
                    filtered_plants.append(plant_data)
                else:
                    distance = calculate_distance_km(plant_lat, plant_lon, 39.9042, 116.4074)
                    logger.debug(f"可再生能源电站 {plant_name} 距离北京 {distance:.1f}km，超出100km范围，跳过")
        ...
        logger.info(f"100km范围内的可再生能源数据: {len(filtered_df)} 条记录，{filtered_df['plant_name'].nunique() if len(filtered_df) > 0 else 0} 个电站")
    ...
    def _process_renewable_data_fallback(self, renewable_data: pd.DataFrame):
        ...
        for plant_name in renewable_data['plant_name'].unique():
            ...
            if len(plant_data) >= self.total_hours:
                ...
                # 检查坐标是否在北京300公里范围内（与DAC、煤制氢模型保持一致）
    ...
    def _load_preprocessed_renewable_data(self) -> Tuple[pd.DataFrame, bool]:
        ...
        try:
            ...
            error_msg = (
                ...
                f"请先运行预处理脚本生成数据：\n"
                f"  cd products/supply_chain_optimization/natural_gas_supply_chain_optimization/scripts\n"
    ...
    def _process_byproduct_h2_source(
        ...
        for _, row in df.iterrows():
            ...
            # 300km范围检查：以北京为中心（与DAC、煤制氢模型保持一致）
            if not is_within_beijing_range(plant_lat, plant_lon, 300):
                filtered_count += 1
                continue
        ...
        try:
            ...
            if len(hydrogen_location_dict) > 0:
                ...
                if use_super_graph and SUPER_GRAPH_AVAILABLE and SuperGraphOptimizer is not None:
                    ...
                    # 构建不包含天然气管道的位置字典（氢气管道的目标位置）
                    # 氢气管道应该连接：氢气源 → MTJ工厂（airport/lng_terminal/renewable_plant）
                    # 不应包含天然气管道接入点（ng_pipeline_*）
                    non_ng_pipeline_locations = {
                        loc: info for loc, info in self.locations.items()
                        if not loc.startswith('ng_pipeline_')  # 排除天然气管道位置
                    }

                    logger.info(f"超图优化器配置:")
                    logger.info(f"  氢气源节点: {len(h2_production_sources)} 个（风电/光伏站点）")
                    logger.info(f"  目标节点: {len(non_ng_pipeline_locations)} 个（排除天然气管道）")
                    logger.info(f"  已排除: {len(self.locations) - len(non_ng_pipeline_locations)} 个天然气管道位置")
                    ...
                    self.super_graph_optimizer = SuperGraphOptimizer(
                        ...
                        co2_capture_sources=h2_production_sources,  # 氢气源（重命名参数更清晰）
                        locations=non_ng_pipeline_locations,  # 不包含天然气管道
    ...
    def _create_cost_expressions(self):
        ...
        # 8. 电力成本（拆分为两部分：H2电解 + MTJ合成）
        # 修正日期：2025-11-09 - 将电力成本拆分为H2电解和MTJ合成两部分
        ...
        # 8.2 MTJ合成工艺电力成本（甲醇制航空燃料）
        # 新增：从配置文件中获取MTJ合成能耗参数（之前仅用于碳排放计算，未计入成本）
        # 修正日期：2025-11-09
        mtj_synthesis_energy_kwh_per_ton = float(
            self.config.get('carbon_emission_parameters', {}).get('production_process', {}).get('mtj_process_energy', 4500)
        )

        # MTJ工艺能耗单位: kWh/t SAF, 需要转换为 kWh/kg
        mtj_synthesis_energy_kwh_per_kg = mtj_synthesis_energy_kwh_per_ton / 1000  # 4500 kWh/t = 4.5 kWh/kg

        # 根据SAF工厂位置类型，使用不同的电价
        # renewable_plant (solar/wind): 使用可再生电价 0.35 yuan/kWh
        # 其他位置 (airport等): 使用电网电价 0.6 yuan/kWh
        renewable_electricity_price_yuan_per_kwh = (
            self.costs['renewable_electricity_cost_yuan_per_mwh'] / 1000  # MWh -> kWh
        )
        ...
        grid_mtj_power_cost = gp.quicksum(
            ...
            for tech in self.technologies
            for hour in range(self.total_hours)
            if self.locations[location]['type'] not in ['solar_plant', 'wind_farm', 'byproduct_hydrogen_steel', 'byproduct_hydrogen_refinery']
        ...
        # 8.3 MTJ催化剂成本（20年生命周期现值）
        # 新增：从配置文件中获取催化剂成本参数（之前未定义）
        # 修正日期：2025-11-09
        # 从technologies配置中获取催化剂成本，需要遍历所有技术定义
        catalyst_costs_by_tech = {}
        for tech_name, tech_config in self.config.get('technologies', {}).items():
            if isinstance(tech_config, dict):
                catalyst_cost = tech_config.get('catalyst_cost_yuan_per_kg_saf', 0.10)  # 默认0.10元/kg
                catalyst_costs_by_tech[tech_name] = catalyst_cost

        # MTJ催化剂成本表达式
        self.cost_expressions['catalyst_cost'] = gp.quicksum(
            self.production_vars[(location, tech, hour)] *
            catalyst_costs_by_tech.get(tech, 0.10)  # 如果技术未找到，使用默认值0.10元/kg
            for location in self.locations
            for tech in self.technologies
            for hour in range(self.total_hours)
            if (location, tech, hour) in self.production_vars
        ...
        logger.info(f"MTJ催化剂成本已添加（默认0.10元/kg SAF）")
        ...
        # 9. 氢气罐车运输投资 + 20年运营成本现值（已禁用，仅使用管道）
        self.cost_expressions['hydrogen_transport_investment'] = gp.LinExpr(0)
        # 【已禁用】氢气罐车运输成本 - 修正日期：2025-11-16
        ...
        # )
        self.cost_expressions['hydrogen_transport_operation'] = gp.LinExpr(0)  # 氢气罐车已禁用
    ...
    def _calculate_dynamic_renewable_intensity(self, location, hour):
        ...
        wind_ci = renewable_energy.get('wind_power_intensity', 0.015)  # kgCO₂e/kWh
        solar_ci = renewable_energy.get('solar_power_intensity', 0.045)  # kgCO₂e/kWh

        # 尝试获取实际发电数据
        # 注意: renewable_generation_data结构需要根据实际数据加载器确定
        # 假设格式: {(location, hour): {'wind_mwh': X, 'solar_mwh': Y}}
    ...
    def _create_carbon_emission_expressions(self):
        ...
        renewable_elec_fixed = production.get('renewable_electricity', 0.02)  # kg CO2eq/kWh (已废弃,仅作回退)
        electrolysis_energy = production.get('electrolysis_energy', 55)  # kWh/kg H2
        ...
        if ci_values:
            ...
            logger.info(f"对比固定值: {renewable_elec_fixed} kgCO₂e/kWh (已废弃)")

        # 计算绿氢动态碳强度 (使用动态电力碳强度重新计算)
        # 原公式: green_h2_intensity = 55 kWh/kg × 0.02 kgCO₂e/kWh + 0.05 kgCO₂e/kg = 1.1
        # 新公式: 动态计算,基于每个location-hour的实际碳强度
        h2_process_emission = production.get('h2_electrolysis_emission', 0.05)  # 电解过程排放
        self.dynamic_h2_intensity = {}  # 缓存: {(location, hour): H2 CI值}
        ...
        logger.info(f"绿氢动态碳强度计算完成 (基于电解能耗{electrolysis_energy} kWh/kg + 过程排放{h2_process_emission})")

        # 天然气制甲醇碳排放 (无需修改,不涉及可再生能源)
        ...
        # - 副产氢站点 (byproduct_hydrogen_steel, byproduct_hydrogen_refinery) → 使用上游生命周期分配 + 提纯压缩电耗碳排放
        ...
        self.carbon_expressions['h2_production'] = gp.quicksum(
            ...
            self.dynamic_h2_intensity.get((location, hour), 1.1)  # 回退值1.1
            for location in self.hydrogen_locations
            for hour in range(self.total_hours)
            if (location, hour) in self.hydrogen_production_vars
        )
        ...
        self.carbon_expressions['mtj_storage'] = gp.quicksum(
            ...
            mtj_storage_energy / 24 * self.dynamic_carbon_intensity.get((location, hour), renewable_elec_fixed)  # kWh/t·天 → kWh/t·h
            for location in self.locations
        ...
        self.carbon_expressions['h2_storage'] = gp.quicksum(
            ...
            h2_storage_energy / 24 * self.dynamic_carbon_intensity.get((location, hour), renewable_elec_fixed)  # 转换为小时级
            for location in self.hydrogen_locations
        ...
        # )
        self.carbon_expressions['h2_truck_transport'] = gp.LinExpr(0)  # 氢气罐车已禁用
        ...
        # =========================================================================
        # E-CRM技术：天然气处理产生的CO₂被固定在甲醇中，最终固定在SAF产品中
        ...
        if enable_co2_credit:
            # 仅当配置允许时计算CO₂利用负排放（适用于工业捕获/DAC场景）
            # 计算方法：SAF产品中的碳含量 × CO₂/C摩尔比(44/12) × 利用率
            ...
            # CO₂利用负排放 = SAF总产量 × 碳含量 × CO₂/C比 × 负排放系数
            ...
            logger.info(f"   SAF碳含量: {saf_carbon_content} kg C/kg SAF")
            logger.info(f"   CO₂/C摩尔比: {co2_to_c_ratio:.2f}")
            logger.info(f"   负排放系数: {utilization_credit_factor} (CORSIA标准)")
            logger.info(f"   计算公式: SAF产量 × {saf_carbon_content} × {co2_to_c_ratio:.2f} × {utilization_credit_factor}")
        else:
            # 化石燃料原料场景（天然气重整）：副产CO₂循环利用不属于碳移除，不计入负排放
    ...
    def _add_hydrogen_transport_capacity_constraints(self):
        """【已禁用】添加氢气罐车运输能力限制约束 - 修正日期：2025-11-16"""
        logger.info("氢气罐车运输已禁用，跳过运输能力约束")
        return
            ...
            if location_type in ['solar_plant', 'wind_farm', 'byproduct_hydrogen_steel', 'byproduct_hydrogen_refinery']:
                ...
                for hour in range(self.total_hours):
                    ...
                    if hour % self.periods_per_week == last_period_in_week:  # 每周最后一个期间扣减运输量
                        ...
                        # )
                        weekly_outbound_transport = gp.LinExpr(0)  # 罐车已禁用
            ...
            # 为MTJ工厂（airport, lng_terminal）添加氢气入库约束
                ...
                for hour in range(self.total_hours):
                    ...
                    if hour % self.hours_per_week == 0:  # 每周第一个小时入库（索引0）
                        ...
                        # )
                        weekly_inbound_transport = gp.LinExpr(0)  # 罐车已禁用
