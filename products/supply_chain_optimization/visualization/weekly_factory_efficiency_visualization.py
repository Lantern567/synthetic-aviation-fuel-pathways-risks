...
Shows per-week utilization for SAF plants (derived from weekly transport_plan) and
...
def compute_saf_weekly_utilization(data):
    ...
    if not weekly_output:
        return []
    weeks = sorted(weekly_output.keys())
    util = []
    for w in weeks:
        kg = weekly_output[w]
...
def main():
    ...
    for name, cfg in SCENARIOS.items():
        ...
        try:
            ...
            continue
        saf_util = compute_saf_weekly_utilization(data)
