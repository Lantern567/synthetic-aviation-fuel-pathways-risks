...
class FastSensitivityAnalyzer:
    ...
    def __init__(self,
        ...
        log_dir = os.path.join(
            self.project_root, "products", "supply_chain_optimization",
            "dac_hydrogen_saf_supply_chain_optimization", "logs"
        )
    ...
    def run_analysis(self):
        ...
        try:
            ...
            sys.path.insert(0, os.path.join(self.project_root, "products",
                                           ...
                                           "dac_hydrogen_saf_supply_chain_optimization", "src"))
            # ===== v4.0变更: 导入DAC版本优化器 =====
            from core.dac_hydrogen_optimization_model import DACHydrogenSAFOptimizer

            # 初始化优化器 (使用1周时间范围)
            # 注意: 第一个参数是config_path，使用None表示使用默认配置
            optimizer = DACHydrogenSAFOptimizer(
