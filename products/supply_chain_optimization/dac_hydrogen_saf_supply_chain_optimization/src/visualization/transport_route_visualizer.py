...
class TransportRouteVisualizer:
    ...
    def load_co2_clustering_data(self):
        ...
        try:
            ...
            # 路径: src/visualization/ -> src/ -> dac_hydrogen_saf_supply_chain_optimization/
            current_dir = Path(__file__).parent
            project_root = current_dir.parent.parent

            # ===== v4.0变更: CO2聚类结果已不适用（DAC版本无需工业源聚类） =====
            # v4.0使用DAC直接捕获，无需加载co2_capture_sources.csv和聚类结果
            # 此方法在v4.0中返回None，不影响其他可视化功能
            ...
            if not co2_clustering_file.exists():
                self.logger.warning(f"CO2聚类结果文件不存在（v4.0 DAC版本中正常）: {co2_clustering_file}")
                return None, None

            if not co2_sources_file.exists():
                self.logger.warning(f"CO2源点数据文件不存在（v4.0 DAC版本已删除）: {co2_sources_file}")
                return None, None
...
总运输量: {total_volume/1e6:.1f}万吨/周"""
    ...
    def _add_pipeline_networks_to_existing_map(self, ax, hydrogen_transport_data=None):
        ...
        # GIS数据路径
        current_dir = Path(__file__).parent
        # 路径: src/visualization/ -> src/ -> dac_hydrogen_saf_supply_chain_optimization/ -> supply_chain_optimization/ -> products/
    ...
    def find_latest_files(self, results_dir=None):
        ...
        if results_dir is None:
            # 路径: src/visualization/ -> src/ -> dac_hydrogen_saf_supply_chain_optimization/ -> results/
