"""
DAC Hydrogen SAF供应链运输路线可视化
DAC Hydrogen SAF Supply Chain Transport Route Visualization

功能 | Features:
1. 氢气管道运输可视化 (Hydrogen pipeline transport visualization)
2. SAF卡车运输可视化 (SAF truck transport visualization)
3. DAC设施位置标注 (DAC facility location marking)
...
class DACHydrogenTransportRouteVisualizer:
    """DAC Hydrogen运输路线可视化器"""
    ...
    def __init__(self, output_dir: str = None):
        ...
        Args:
            output_dir: 输出目录，默认为 dac_hydrogen_saf_supply_chain_optimization/results/visualization
        """
    ...
    def create_base_map(self, figsize=(14, 10)):
        ...
        # 设置地图范围 - 京津冀及周边地区
        main_ax.set_extent([111, 121, 35, 43], crs=self.data_crs)
    ...
    def plot_facilities(self, ax):
        ...
        for facility_name, facility_info in facilities.items():
            ...
            elif 'co2_capture' in location_type.lower() or 'dac' in location_type.lower():
                facility_types['dac'].append((lon, lat, facility_name))
            elif 'airport' in location_type.lower():
                facility_types['airport'].append((lon, lat, facility_name))
                # 注意：DAC模块中某些位置既是MTJ工厂也是其他设施
                if 'green_h2_co2' in technology:
                    facility_types['mtj'].append((lon, lat, facility_name))
        ...
        for ftype, facilities_list in facility_types.items():
            ...
            ax.scatter(lons, lats,
                      ...
                      s=120 if ftype == 'dac' else (100 if ftype == 'airport' else 50),
                      alpha=0.8,
                      edgecolors='black',
                      linewidth=0.5,
    ...
    def create_comprehensive_map(self):
        ...
        # 添加标题
        ax.set_title('DAC Hydrogen SAF供应链运输路线图\nDAC Hydrogen SAF Supply Chain Transport Routes',
    ...
    def create_transport_flow_analysis(self):
        ...
        fig.suptitle('DAC Hydrogen运输流量分析 | Transport Flow Analysis',
        ...
        if self.mtj_transport_df is not None and len(self.mtj_transport_df) > 0:
            mtj_sorted = self.mtj_transport_df.sort_values('运输量(kg)', ascending=True).tail(min(10, len(self.mtj_transport_df)))
            ...
            ax2.set_title(f'SAF运输路线Top {len(labels)} | Top SAF Transport Routes', fontsize=11, fontweight='bold')
    ...
    def generate_summary_report(self):
        ...
        summary = {
            ...
            'DAC设施投资(亿元)': self.solution_data.get('dac_facility_investment', 0) / 1e9 if self.solution_data else 0,
            'DAC捕获成本(亿元)': self.solution_data.get('dac_capture_cost', 0) / 1e9 if self.solution_data else 0,
            'CO2管道运输成本(亿元)': self.solution_data.get('co2_pipeline_transport_cost', 0) / 1e9 if self.solution_data else 0
        }
...
def main():
    """主函数"""
    logger.info("=" * 60)
    logger.info("DAC Hydrogen运输路线可视化脚本")
    logger.info("DAC Hydrogen Transport Route Visualization Script")
    logger.info("=" * 60)

    try:
        # 创建可视化器
        visualizer = DACHydrogenTransportRouteVisualizer()

        # 设置文件路径（使用最新结果）
        base_dir = Path(__file__).parent.parent.parent / "results" / "two_step"
        solution_file = base_dir / "complete_solution_20251106_015557.json"
        h2_transport_file = base_dir / "hydrogen_transport_plan_20251106_015557.csv"
        mtj_transport_file = base_dir / "mtj_transport_plan_20251106_015557.csv"
