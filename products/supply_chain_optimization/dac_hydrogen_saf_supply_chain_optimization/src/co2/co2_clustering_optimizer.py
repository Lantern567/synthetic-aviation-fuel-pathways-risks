"""
CO2捕获源聚类优化器（DAC版本）
基于DBSCAN算法对DAC设施进行地理聚类
优化聚类中心点位置以最小化管道运输成本

v4.1变更：
- 从工业点源捕获改为DAC独立设施捕获
- 输入数据从CO2捕获源改为DAC候选位置
- 聚类对象从燃煤电厂等改为可再生能源站点和机场
"""
...
try:
    ...
    logging.warning("Numba不可用，将使用NumPy实现（速度较慢）。建议: pip install numba")

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# 禁用Numba缓存以避免Windows下的文件系统问题
# 注意：禁用缓存后，首次运行会稍慢（需要JIT编译），但避免了缓存文件写入错误
NUMBA_CACHE_ENABLED = False
...
if NUMBA_AVAILABLE:
    @jit(nopython=True, parallel=True, cache=NUMBA_CACHE_ENABLED)
...
class CO2Cluster:
    """DAC聚类结果数据类"""
    cluster_id: int
    member_locations: List[str]  # DAC设施位置名称列表
    member_coords: List[Tuple[float, float]]  # DAC设施坐标列表
    center_coord: Tuple[float, float]  # 聚类中心坐标（优化后）
    geo_center: Tuple[float, float]  # 地理中心坐标
    pipeline_optimized_center: Optional[Tuple[float, float]]  # 管道优化后的中心
    total_capacity_kg_per_week: float  # 总DAC捕获容量（kg/周）

@dataclass
class CO2ClusteringResult:
    """DAC聚类结果"""
    clusters: List[CO2Cluster]
    noise_points: List[Tuple[str, Tuple[float, float]]]  # 独立DAC设施
    total_clusters: int
...
class CO2ClusteringOptimizer:
    ...
    def __init__(self, config: Dict, pipeline_distance_calculator=None):
        ...
        # DAC聚类参数
        self.eps_km = self.clustering_params.get('eps_distance_km', 30.0)  # 30km
        ...
        self.max_clusters = self.clustering_params.get('max_clusters', 200)

        # 🚀 性能优化：管道距离缓存
        self._pipeline_distance_cache = {}

        logger.info(f"初始化DAC聚类优化器: eps={self.eps_km}km, min_samples={self.min_samples}, max_clusters={self.max_clusters}")
    ...
    def _dbscan_cluster(self, coords_rad: np.ndarray, eps_rad: float, min_samples: int) -> np.ndarray:
        ...
        logger.info(f"开始DBSCAN聚类: {n_points}个DAC候选点, eps={eps_rad*6371:.1f}km, min_samples={min_samples}")
    ...
    def cluster_dac_facilities(self, dac_locations: Dict,
                               saf_factory_coords: Optional[List[Tuple[float, float]]] = None) -> CO2ClusteringResult:
        """
        对DAC设施候选位置进行聚类

        Args:
            dac_locations: DAC候选位置字典 {location_id: {latitude, longitude, type, ...}}
            saf_factory_coords: SAF工厂位置坐标列表（用于优化聚类中心）

        Returns:
            CO2ClusteringResult: DAC聚类结果
        """
        t_total_start = time.time()

        if not self.clustering_params.get('enable_clustering', False):
            logger.info("DAC聚类功能未启用，跳过聚类")
            noise_points = [(loc, (coords['latitude'], coords['longitude']))
                           for loc, coords in dac_locations.items()]
        ...
        logger.info(f"=" * 60)
        logger.info(f"开始DAC设施聚类分析: {len(dac_locations)}个候选位置")
        logger.info(f"=" * 60)

        # 🚀 阶段1：数据准备
        t_prep_start = time.time()
        location_names = list(dac_locations.keys())
        coords = np.array([(dac_locations[loc]['latitude'],
                           dac_locations[loc]['longitude'])
        ...
        logger.info(f"✓ 聚类结果: {n_clusters}个聚类, {n_noise}个独立DAC设施")
        ...
        for label in unique_labels:
            ...
            # 管道优化（如果启用且有SAF工厂坐标）
            if self.enable_pipeline_opt and saf_factory_coords:
                # 对每个SAF工厂位置优化聚类中心，选择总距离最小的
                optimized_center = self._optimize_cluster_center_multi_destination(
                    cluster_coords, geo_center, saf_factory_coords
                )
            else:
                optimized_center = geo_center

            # v4.1注意：DAC容量未在此处计算，由优化模型决策变量决定
            # 这里设置为0，表示容量由模型决定
            total_capacity = 0.0
            ...
            cluster = CO2Cluster(
                ...
                total_capacity_kg_per_week=total_capacity
            )
        ...
        if len(clusters) > self.max_clusters:
            logger.warning(f"DAC聚类数量({len(clusters)})超过限制({self.max_clusters})，保留成员数最多的{self.max_clusters}个")
            clusters.sort(key=lambda c: len(c.member_locations), reverse=True)
        ...
        logger.info("=" * 60)
        logger.info(f"✓ DAC聚类全流程完成!")
        logger.info(f"  - 总耗时: {t_total_end-t_total_start:.2f}秒")
        logger.info(f"  - 最终聚类: {result.total_clusters}个, 独立设施: {result.total_noise_points}个")
        logger.info(f"=" * 60)
    ...
    def cluster_co2_sources(self, co2_sources: Dict,
                            destination_coords: Optional[object] = None) -> CO2ClusteringResult:
        """
        兼容旧版接口：对CO₂捕获源进行聚类。

        destination_coords 可以为:
        - None：不考虑终点
        - (lat, lon)：单个终点坐标
        - [(lat1, lon1), (lat2, lon2), ...]：多个终点坐标
        """
        saf_coords: Optional[List[Tuple[float, float]]] = None
        if destination_coords:
            if isinstance(destination_coords, (list, tuple)):
                if len(destination_coords) == 2 and isinstance(destination_coords[0], (int, float)):
                    saf_coords = [tuple(destination_coords)]
                else:
                    saf_coords = [
                        tuple(coord) for coord in destination_coords
                        if isinstance(coord, (list, tuple)) and len(coord) == 2
                    ]
                    if not saf_coords:
                        saf_coords = None
            else:
                logger.debug("destination_coords 类型不受支持，忽略终点坐标输入")

        return self.cluster_dac_facilities(co2_sources, saf_factory_coords=saf_coords)
    ...
    def _optimize_cluster_center_multi_destination(self, cluster_coords: List[Tuple[float, float]],
                                                    geo_center: Tuple[float, float],
                                                    destinations: List[Tuple[float, float]]) -> Tuple[float, float]:
        """
        优化聚类中心位置（考虑多个SAF工厂位置）

        Args:
            cluster_coords: DAC聚类成员坐标
            geo_center: 地理中心
            destinations: SAF工厂位置列表

        Returns:
            优化后的中心坐标
        """
        # 简化策略：选择到所有SAF工厂距离之和最小的点
        # 候选点：地理中心 + 算术平均中心 + 部分成员点
        ...
        for point in sample_points:
            # 到所有SAF工厂的总距离
            dest_dist_sum = sum(
                self._calculate_haversine_distance(point[0], point[1], d[0], d[1])
            ...
            score = (1 - self.pipeline_weight) * member_dist_sum + \
                   self.pipeline_weight * dest_dist_sum

            if score < best_score:
                best_score = score
                best_point = point

        logger.debug(f"DAC聚类中心优化: 地理中心{geo_center} -> 优化中心{best_point}")
        return best_point

    def _export_results(self, result: CO2ClusteringResult):
        """导出DAC聚类结果到JSON文件"""
        output_path = self.clustering_params.get('clustering_output_path', 'dac_clustering_results.json')
            ...
            'clusters': [
                {
                    ...
                    'total_capacity_kg_per_week': c.total_capacity_kg_per_week,
                    'member_count': len(c.member_locations)
                }
        ...
        logger.info(f"DAC聚类结果已导出到: {output_file.absolute()}")
