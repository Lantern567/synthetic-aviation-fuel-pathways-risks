...
废弃日期：2025-11-09
迁移到：coal_hydrogen_optimization_model.py 的 _create_carbon_emission_expressions() 方法
...
煤制氢+CO₂制SAF碳排放量计算器 (Coal-based H2 + CO2 to SAF Emission Calculator)

本模块计算煤制氢+CO₂制SAF全生命周期碳排放，包括：
1. 原材料排放（煤制氢生产、CO₂捕获过程）
2. 运输排放（H₂、CO₂、SAF运输）
...
作者：Claude Code
创建日期：2025-10-13
修改日期：2025-11-09 (标记为废弃)
版本：v3.0（已废弃 - DEPRECATED）

参考文档：
- 煤制氢供应链优化产品需求文档_PRD_v3.0.md
- IPCC碳排放计算指南
- CORSIA可持续航空燃料标准
"""
...
warnings.warn(
    ...
    "所有碳排放计算已迁移到 coal_hydrogen_optimization_model.py 的 _create_carbon_emission_expressions() 方法中。"
    "详见模块顶部的废弃说明。",
    DeprecationWarning,
    stacklevel=2
)
