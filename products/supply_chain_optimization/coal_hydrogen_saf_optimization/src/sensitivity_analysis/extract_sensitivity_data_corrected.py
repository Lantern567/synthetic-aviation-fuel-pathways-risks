...
def extract_data_from_solution(solution_file: Path, run_index: int, param_value: float) -> dict:
    ...
    try:
        ...
        # 提取顶层字段
        lifecycle_cost = data.get('lifecycle_levelized_cost_per_kg', 0)
...
def main():
    ...
    if valid_count > 0:
        ...
        logger.info(f"  平准化成本: {valid_df['lifecycle_levelized_cost'].min():.2f} - {valid_df['lifecycle_levelized_cost'].max():.2f}")
    ...
    try:
        ...
        if old_results_file.exists():
            backup_file = processed_data_dir / "results_old.csv"
            old_results_file.rename(backup_file)
            logger.info(f"已备份旧文件: results_old.csv")
